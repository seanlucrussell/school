...
public:
  ...
  inline void setX(double newX) {x = newX;}          //!< Return the X coordinate of the point
  inline void setY(double newY) {y = newY;}          //!< Return the Y coordinate of the point
  inline void setZ(double newZ) {z = newZ;}          //!< Return the Z coordindate of the point

  void scale(double scale);			//!< Vector scale the point by some scalar
  void transform(double xTransform, double yTransform, double zTransform);
  double pointCompare(const Point3D p) const;
