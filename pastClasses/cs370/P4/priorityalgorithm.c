...
// find process with lowest arrival time firstly, then lowest burst time
  ...
  for(int k=0;k<lc;k++){
    ...
    for(int i=smallest;i<lc;++i){
      ...
      if (processList[i].priority<processList[smallest].priority
                         || (processList[i].priority<=processList[smallest].priority
    ...
    listLocation++;
    printf("P%d\t%d\t%d\t%d\t%.1f\t%.1f\n",processList[smallest].id,processList[smallest].arrivalTime,processList[smallest].burstDuration,processList[smallest].priority,waitTime,tat);
