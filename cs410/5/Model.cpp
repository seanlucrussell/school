...
void Model::ApplyTransformation(float wx, float wy, float wz, float theta,
...
bool Model::ParseLine(const string& line, vector<Vector4d>& vertices) {
  ...
  if (tokens.size() > 0) {
    ...
    if (tokens[0] == "f") {
      ...
      Face facePair(face, material);
      faces.push_back(facePair);
    }
    if (tokens[0] == "mtllib") {
      material.ReadFromFile(tokens[1]);
      materialLib = material.ReadMaterialLibFromFile(tokens[1]);
    }
    if (tokens[0] == "usemtl") {
      material = materialLib[tokens[1]];
    }
  }
}

bool Model::ReadFromFile(const string& fileName) {
